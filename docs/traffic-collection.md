...
- TC ingress map keys contain `source_ip` as the source IPv4 address,
  ...
  protocol number.
- Counters contain packet totals and byte totals. All three programs count
  IPv4 `tot_len`, L3 and above only; the Ethernet header is excluded.
...
- All hash writes and expirations for an emitted second are sent through one
  Redis pipeline before their replies are checked.
